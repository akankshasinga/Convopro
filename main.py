...
from services.get_models_list import get_ollama_models_list
from services.get_title import get_chat_title
from services.chat_utilities import get_answer, stream_answer
from db.conversations import (
    create_new_conversation,
    add_message,
    get_conversation,
...
if user_query:
    ...
    # 3) STREAM assistant response (FIXED)
    assistant_text = ""
    with st.chat_message("assistant"):
        response_box = st.empty()
        try:
            for token in stream_answer(selected_model, st.session_state.chat_history):
